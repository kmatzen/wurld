...
export class WurldLivePlayer {
  ...
  _consumeTags(elem) {
    ...
    if (typeof tags.WURLD === 'string') {
      this.doc = JSON.parse(tags.WURLD);
    ...
    if (tags.WURLD_POSES instanceof Uint8Array) {
      const chunkFrames = unpackFrames(tags.WURLD_POSES, this._cameraKeys);
      this.frames.push(...chunkFrames);
      this._onPoses?.(chunkFrames);
    }
    if (tags.WURLD_FRAMES instanceof Uint8Array) {
      this.frames = unpackFrames(tags.WURLD_FRAMES, this._cameraKeys);
      this._onPoses?.(this.frames);
    }
    return true;
  }
